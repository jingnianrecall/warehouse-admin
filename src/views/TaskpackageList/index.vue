<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      <el-input placeholder="请输入描述信息进行查询" v-model="listQuery.describe" style="width: 300px;" class="filter-item" @keyup.enter.native="handleFilter"/>
    ...
    <el-table
      ...
      <el-table-column prop="id" align="center" sortable label="ID" width="65">
        <template slot-scope="scope">
          {{ scope.row.id }}
        </template>
      </el-table-column>
      <el-table-column prop="name" label="任务包名称" sortable width="150">
        <template slot-scope="scope">
          {{ scope.row.name }}
        </template>
      </el-table-column>
      <el-table-column prop="nums" label="图幅数" sortable width="89">
        <template slot-scope="scope">
          {{ scope.row.nums }}
        </template>
      </el-table-column>
      <el-table-column prop="owner" label="作业员" sortable width="100" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column align="center" prop="create_time" sortable label="最后更新时间" width="160">
        <template slot-scope="scope">
      ...
      <el-form
        ...
        <el-form-item>
          ...
          <uploader
            ...
            :Browse="browse"
          />
          <el-button type="primary" id="browse_button">上传</el-button>
        </el-form-item>
      </el-form>
    ...
    <el-dialog :title="atFunTitle" :visible.sync="dialogAtFunVisible">
      <el-form ref="atFunForm" :model="atFunForm" :rules="atFunRules">
        ...
        <el-form-item label="描述" prop="describe" label-width="120px">
          <el-input v-model="atFunForm.describe" placeholder="请输入描述" ></el-input>
        </el-form-item>
      </el-form>
    ...
    <!-- @记录列表 -->
    <el-dialog title="@记录" :visible.sync="dialogAtRecVisible">
...
export default {
  ...
  methods: {
    ...
    handleData: function(subversionListQuery, id, name) {
      ...
      subversionListQuery.name = this.dataMGMTTitle
    ...
    atOperatorRecord(atRecListQuery, id, name) {
      ...
      atRecListQuery.name = this.dataMGMTTitle
